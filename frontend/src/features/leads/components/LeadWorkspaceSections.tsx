...
export function LeadWorkspaceSection({
  title,
  icon: Icon,
  children,
  className,
...
export function LeadWorkspaceMetric({
  label,
  value,
}: {
  label: string;
...
export function LeadContactPanel({
  ...
  const rows = [
    ...
    },
    {
      ...
      value: email || t("leads.emailMissing"),
    },
    {
      icon: Inbox,
      label: t("leads.source"),
      value: getSourceLabel(lead.source, t),
    },
    {
      icon: UserRound,
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid gap-2 text-sm">
        <MetaRow
          label={t("leads.nextStep")}
          value={lead.recommended_action || nextAction(lead, t)}
        />
        <MetaRow
...
function Score({ label, value }: { label: string; value: number | string }) {
  return (
    <div className="rounded-control border border-[#E6DDD2] bg-white px-3 py-2">
      <p className="text-xs font-semibold text-[#8A7B70]">{label}</p>
      <p className="mt-1 text-xl font-bold text-zani-text">{value}</p>
    </div>
  );
}
...
export function leadAvatarLabel(
  lead: Lead,
  client: Client | null,
  fallback: string,
) {
