          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <div className="mb-2 flex flex-wrap items-center gap-2">
                ...
                <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-500">{getChannelLabel(lead.source, t)}</span>
                {serviceName ? <span className="rounded-full bg-brand-50 px-3 py-1 text-xs font-black text-brand-700">{serviceName}</span> : null}
              </div>
              <h3 className="truncate text-xl font-black text-midnight">{client?.full_name || lead.client_name || t("crmCard.leadNumber", { id: lead.id })}</h3>
