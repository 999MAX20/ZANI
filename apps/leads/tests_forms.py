...
class LeadFormCaptureTests(TestCase):
    ...
    def test_public_form_creates_client_lead_submission_and_runs_automation(self):
        ...
        client = Client.objects.get(business=self.business)
        self.assertEqual(client.source_detail, "spring")
        self.assertEqual(client.source_context_json["campaign"], "spring")
        ...
        event = BusinessEvent.objects.get(business=self.business, source="website", event_type="lead.captured")
        self.assertEqual(event.payload_json["submission_id"], submission.id)
        self.assertEqual(event.payload_json["lead_id"], Lead.objects.get(business=self.business).id)
        self.assertEqual(event.payload_json["client_id"], client.id)
