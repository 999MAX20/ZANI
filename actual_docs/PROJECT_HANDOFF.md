...
## Последующее решение: тарифы и биллинг отложены — 2026-09-25

[Краткая заметка обсуждения](../docs/billing/BILLING_DISCUSSION_DEFERRED_2026-09-25.md)
заменяет прежнее коммерческое решение «PAYG сохранён / ИИ вне подписки» ниже:
пакет CRM включает AI-объём, его исчерпание приостанавливает AI, а не оплаченную
ручную CRM. Примеры цен не утверждены; работа отложена, денежный gate пилота и
GRACE/READ_ONLY по окончании CRM сохранены. Остальные правила и evidence не меняются.
Новая продуктовая фаза не запущена; публикация этой записи — в верхнем checkpoint.

## Ответы владельца по пилоту — 2026-09-25

[Решения шести аннотаций](../docs/product/V1_PRODUCT_RULES.md#owner-decisions-20260925)
уточняют оплачиваемый CRM-аккаунт и отдельные пороги специалистов, процессинг
подписки Platforma, логическую AI-операцию/отдельные cost и charge, основную
policy ACTIVE/3 дня GRACE/READ_ONLY и общую multi-tenant эксплуатацию.
Billing/AI/infra контракты и [план](../docs/pilot/local-crm-completion.md#current-plan)
отделяют эти требования от примеров и оставшихся решений. AI-пакеты не заменяют
PAYG; цены/веса/provider и спорные края не придуманы. A/B/C-разбор в AGENTS
не меняет ручное переназначение, независимость login/специалиста и staff-action.
Только документация; анализ тарифов, billing/infra и CRM-приёмка не запущены.
Scope, проверки и публикация — в верхней записи PRIMARY-SESSION.

## План платного пилота — актуализация 2026-09-25

[Единый существующий план](../docs/pilot/local-crm-completion.md#current-plan)
показывает состав V1 для одной реальной клиники, доказанные закрытия, остаток
по типу разрыва, зависимости и решения владельца. Сначала рекомендована
приёмка внутренних CRM-циклов с исправлением только доказанных пробелов.
Это предложение следующего bounded этапа, не его запуск. Текущее поручение —
только документация; точная проверка и публикация в верхней записи PRIMARY-SESSION.
Базовый состав сохраняет WhatsApp, Instagram, Telegram и форму сайта;
исключение Telegram/формы для первой клиники не утверждено.

## Завершённая передача — 2026-09-25

Передача 2026-09-25 поколения 1 → 2 завершена: Source DoD и понимание преемника
проверены, архив source подтверждён нативно, transition idle. Новый primary —
01a0d6c1-3ee4-7f92-b94c-0242bb2eb35e; source остаётся retired. Ключ перехода,
доказательства и публикационная квитанция — в разделе «Explicit handoff
generation 1 → 2» PRIMARY-SESSION.
Следующий продуктовый scope владелец выбирает отдельно; он ещё не согласован.

## Управляемая передача — решение владельца 2026-09-25

Настроена команда **«Передай работу новому чату»**. Текущий primary следует
[протоколу](../docs/testing/SESSION_ROLLOVER.md): один преемник Platforma.CRM
в том же проекте/локальной папке, подготовленное сообщение, проверка понимания,
переключение владельца, архив source и native readback. Это инструкция агенту;
hook/наблюдатель за ручной архивацией не включён. Сама настройка не создавала чат
и не меняла реестр; последующая явная команда выполнила переход, указанный выше.
Текущее состояние — [STATUS](../STATUS.md), evidence — верхняя запись
[PRIMARY-SESSION](../docs/testing/task-state/PRIMARY-SESSION.md).
Исторические пакеты ниже не задают очередь и не запускают передачу автоматически.
...
тарифами, без денежного потолка/жёсткого пакета. Уточнение 25.09: seat — аккаунт
с ролью, специалист без аккаунта не seat; AI unit — завершённое полезное действие.
Цены/веса/провайдер и пограничные детали остаются открытыми. Основная policy
ACTIVE/3 дня GRACE/READ_ONLY принята в V1-B04, включая сохранение входящих и
системных операций; это требования, не доказательство live billing.
