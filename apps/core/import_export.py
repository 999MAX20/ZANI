...
DEAL_FIELD_ALIASES = {
    "external_id": ["external_id", "deal_id", "opportunity_id", "id"],
    "title": ["title", "deal", "deal_title", "name", "opportunity"],
    "client_name": ["client_name", "client", "customer", "contact", "full_name", "name"],
    "phone": ["phone", "mobile", "whatsapp"],
    "email": ["email", "e-mail"],
    "amount": ["amount", "value", "sum", "total", "budget"],
    "currency": ["currency"],
...
FIELD_ALIASES_BY_ENTITY = {
    ...
    ImportJob.EntityTypes.DEALS: DEAL_FIELD_ALIASES,
...
FIELDS_BY_ENTITY = {
    ...
    ImportJob.EntityTypes.DEALS: list(DEAL_FIELD_ALIASES.keys()),
...
IMPORT_TEMPLATES = {
    ...
    ImportJob.EntityTypes.DEALS: {
        "headers": list(DEAL_FIELD_ALIASES.keys()),
        "row": ["deal-001", "Imported deal", "Aliya Ivanova", "+77015550101", "aliya@example.com", "15000", "KZT", "New", "manual", "Imported from CSV"],
    },
    ...
    try:
        for row in rows:
            ...
            if job.entity_type == ImportJob.EntityTypes.CLIENTS:
                result = import_client_row(job, payload, connector=connector)
            elif job.entity_type == ImportJob.EntityTypes.LEADS:
                result = import_lead_row(job, payload, connector=connector)
            elif job.entity_type == ImportJob.EntityTypes.DEALS:
                result = import_deal_row(job, payload, connector=connector)
...
def import_client_row(job, payload, connector=None):
    ...
    if existing:
        ...
        if changed:
            record_import_event(
                business=job.business,
                connector=connector,
                event_type=CLIENT_IMPORTED_EVENT,
                external_id=f"client:{existing.id}:{stable_import_row_key(payload)}",
                payload={"client_id": existing.id, "status": "updated", **payload},
            )
        return "updated" if changed else "skipped"
    client = Client.objects.create(business=job.business, **client_defaults(payload))
    record_import_event(
        business=job.business,
        connector=connector,
        event_type=CLIENT_IMPORTED_EVENT,
        external_id=f"client:{client.id}:{stable_import_row_key(payload)}",
        payload={"client_id": client.id, "status": "created", **payload},
    )
    return "created"


def import_lead_row(job, payload, connector=None):
    ...
    record_import_event(
        business=job.business,
        connector=connector,
        event_type=LEAD_IMPORTED_EVENT,
        external_id=f"lead:{lead.id}:{stable_import_row_key(payload)}",
        payload={"lead_id": lead.id, "client_id": client.id, "service_id": service.id if service else None, **payload},
    )
    return "created"
...
def import_deal_row(job, payload, connector=None):
    ...
    client = find_import_client(job.business, {"phone": deal_payload.get("phone"), "email": deal_payload.get("email")})
    if client:
        fill_missing_client_fields(
            client,
            {
                "full_name": deal_payload.get("client_name"),
                "phone": deal_payload.get("phone"),
                "email": deal_payload.get("email"),
                "source": deal_payload.get("source"),
                "notes": deal_payload.get("notes"),
            },
        )
    else:
        client = Client.objects.create(
            ...
            **client_defaults(
                {
                    "full_name": deal_payload.get("client_name") or deal_payload.get("title"),
                    "phone": deal_payload.get("phone"),
                    "email": deal_payload.get("email"),
                    "source": deal_payload.get("source"),
                    "notes": deal_payload.get("notes"),
                },
                fallback="Imported deal client",
            ),
        )
    pipeline = ensure_default_pipeline(job.business)
    stage = default_deal_import_stage(pipeline, deal_payload.get("stage"))
    title = deal_payload.get("title") or f"Imported deal - {client.full_name}"
    duplicate = Deal.objects.filter(
        business=job.business,
        client=client,
        title=title,
        amount=deal_payload["amount"],
        source=deal_payload.get("source", ""),
        is_archived=False,
    ).first()
    if duplicate:
        return "skipped"
    owner = job.actor if job.actor_id and job.business.members.filter(user=job.actor, is_active=True).exists() else None
    deal = Deal.objects.create(
        ...
        title=title,
        amount=deal_payload["amount"],
        currency=deal_payload["currency"],
    ...
    event_payload = {**deal_payload, "amount": str(deal_payload["amount"])}
    record_import_event(
        business=job.business,
        connector=connector,
        event_type=DEAL_IMPORTED_EVENT,
        external_id=deal_payload.get("external_id") or f"deal:{deal.id}:{stable_import_row_key(event_payload)}",
        payload={
            **event_payload,
            "deal_id": deal.id,
            "client_id": client.id,
...
def normalize_deal_payload(payload):
    amount = parse_decimal(payload.get("amount")) if payload.get("amount") else Decimal("0")
    if amount is None:
        raise ValidationError({"amount": "Amount must be a number."})
    currency = str(payload.get("currency") or "KZT").strip().upper()[:8] or "KZT"
    return {
        "external_id": payload.get("external_id", ""),
...
def default_deal_import_stage(pipeline, stage_name=""):
    stages = pipeline.stages.filter(business=pipeline.business)
    if stage_name:
        matched = stages.filter(name__iexact=stage_name, is_won=False, is_lost=False).order_by("order", "id").first()
        if matched:
            return matched
    return stages.filter(is_won=False, is_lost=False).order_by("order", "id").first() or stages.order_by("order", "id").first()
