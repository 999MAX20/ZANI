import { CheckCircle2, CircleAlert, CircleDashed, Clock3, Pause, Play, Plus } from "lucide-react";
  ...
  return (
    <div className="border-b border-slate-100 bg-white px-5 py-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="grid w-full gap-2 sm:grid-cols-2 lg:w-auto lg:grid-cols-[150px_160px_190px_150px_160px]">
          <Select
...
function RelatedEntities({ task, onOpenEntity }: { task: Task; onOpenEntity: (entity: CrmDrawerEntity) => void }) {
  ...
  const entities = [
    ...
    task.appointment
      ...
      : null,
  ].filter(Boolean) as Array<{ key: string; label: string; entity: CrmDrawerEntity }>;
  ...
  return (
    <>
      {visibleEntities.map((item) => (
        <RelatedAction key={item.key} onClick={() => onOpenEntity(item.entity)}>
          {item.label}
        </RelatedAction>
      ))}
