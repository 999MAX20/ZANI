  ...
  return (
    <div className="relative border-b border-slate-100 p-3">
      <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_44px] gap-2">
        <Select
          className="min-h-10 rounded-xl px-2.5 text-xs font-black text-midnight"
          value={queueValue}
        ...
        <button
          type="button"
          className={cn(
            "relative grid h-10 w-11 place-items-center rounded-xl border border-slate-200 bg-white text-slate-600 shadow-sm transition hover:border-brand-200 hover:bg-slate-50",
        ...
        >
          ...
          {advancedCount ? (
            <span className="absolute -right-1 -top-1 grid h-5 min-w-5 place-items-center rounded-full bg-brand-600 px-1 text-[10px] font-black text-white">
              {advancedCount}
            </span>
          ) : null}
        </button>
      ...
      {activeFilterSummary.length ? (
        <div className="mt-2 flex items-center gap-1.5 overflow-x-auto pb-0.5">
          {activeFilterSummary.slice(0, 3).map((label) => (
            <span key={label} className="shrink-0 rounded-full bg-slate-100 px-2 py-1 text-[10px] font-black text-slate-600">
              {label}
            </span>
          ))}
          ...
          <button
            type="button"
            className="ml-auto inline-flex shrink-0 items-center gap-1 rounded-full px-2 py-1 text-[10px] font-black text-brand-600 hover:bg-brand-50"
            onClick={onReset}
          >
            <X size={12} />
            {labels.resetFilters}
      ...
      {isAdvancedOpen ? (
        <div className="absolute left-3 right-3 top-[calc(100%+8px)] z-40 rounded-2xl border border-slate-200 bg-white p-3 shadow-premium">
          <div className="mb-3 flex items-center justify-between gap-3">
            <p className="text-xs font-black uppercase tracking-[0.12em] text-slate-400">{labels.advancedFilters}</p>
            {hasActiveFilters ? (
              <button type="button" className="text-xs font-black text-brand-600" onClick={onReset}>
                {labels.resetFilters}
              </button>
            ) : null}
          </div>
