...
def submit_lead_form(*, lead_form, payload, request=None):
    ...
    source_detail = _source_detail(lead_form=lead_form, payload=payload, source_context=source_context)
    ...
    with transaction.atomic():
        if duplicates:
            client = duplicates[0]
            _fill_missing_client_source_attribution(client, source_detail=source_detail, source_context=source_context)
        else:
        ...
        record_lead_captured_event(
            lead=lead,
            client=client,
            external_id=f"lead-form:{submission.id}",
            payload={
                "lead_form_id": lead_form.id,
                "lead_form_public_id": str(lead_form.public_id),
...
def _source_detail(*, lead_form, payload, source_context):
    detail = (
        payload.get("source_detail")
        or payload.get("campaign")
        or payload.get("utm_campaign")
        or source_context.get("page_domain")
        or lead_form.name
    )
    return _truncate(str(detail or "").strip(), 255)


def _fill_missing_client_source_attribution(client, *, source_detail, source_context):
    update_fields = []
    if source_detail and not client.source_detail:
        client.source_detail = source_detail
        update_fields.append("source_detail")
    if source_context and not client.source_context_json:
        client.source_context_json = source_context
        update_fields.append("source_context_json")
    if update_fields:
