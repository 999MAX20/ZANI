...
### Resource setup UX hardening

Статус: **готово**.

Что изменено:

- На странице `Сотрудники и ресурсы` добавлены быстрые шаблоны создания:
  - мастер / сотрудник;
  - кресло / рабочее место;
  - кабинет;
  - бокс.
- Шаблон сразу подставляет логичный тип и стартовое имя, чтобы владелец салона или сервиса не разбирался в технической модели ресурсов.
- Существующая логика ресурсов и календаря не менялась: ресурс остаётся универсальной сущностью для мастера, кабинета, места или оборудования.
- Все новые тексты добавлены в RU / KK / EN.
...
### Navigation / AI Navigator / Role-Based UX plan 25.05

Статус: **в работе, базовый frontend слой реализован**.

Что изменено:

- Sidebar упрощён до рабочих целей: главная, заявки, сделки, клиенты, сообщения, задачи, аналитика, подключения и настройки.
- Редкие разделы больше не перегружают меню, но прямые маршруты сохранены: услуги, ресурсы, график, календарь, боты, автоматизации и служебные страницы открываются из контекстных блоков.
- Dashboard перенаправляет быстрые действия в актуальные разделы: календарь, подключения и настройки.
- Добавлен общий компонент AI-подсказок для страниц. Подсказки встроены в заявки, клиентов, задачи и аналитику без режима “чат ради чата”.
- Настройки получили операционный центр для услуг, ресурсов и графика работы.
- Подключения сгруппированы по бизнес-смыслу: сообщения, импорт, продажи, сайт и учёт.
- Карточка клиента получила decision snapshot: следующее действие, последнее событие и последние сообщения.
- Sidebar теперь показывает режим интерфейса по роли: владелец, менеджер, оператор или ограниченный сотрудник.
- Универсальные таблицы на мобильных показываются карточками, а не сжатой desktop-таблицей.
...
Full backend tests after settings constants i18n cleanup: 293 OK
Frontend production build after settings constants i18n cleanup: OK
...
AI Assistant chat layout pass: the AI Assistant page now uses a ChatGPT-style workspace with the main chat thread on the left and request history on the right. Business memory, AI-to-task actions and quick prompts stay available in the right context rail without washing out the main content.
...
Production UI/UX references pass started: added `plan/ZANI_UI_UX_PRODUCTION_MASTER_PLAN_26_05.md` as the current phased UI/UX implementation plan based on `references/` and `plan/ZANI_UI_REFERENCES_DEEP_ANALYSIS_26_05_UPDATED.md`. P0 introduced shared production primitives for KPI cards, chips, segmented controls, mobile FAB, right panels, bottom sheets and entity list rows. P1 adjusted the owner dashboard hero toward the light business-cockpit reference style and avoids presenting missing revenue as successful zero revenue. P2.1 started the leads page conversion to a list-first daily-work surface with chips, source filters, list/kanban toggle and mobile FAB while preserving the existing kanban and lead business actions. P2.2 converted the clients page from a table-like directory into a daily-work client list with quick filters, source chips, status pills, next-step cues, WhatsApp action, compact actions menu and mobile FAB while keeping client forms, tags, segments, archive and CRM drawer flows intact. P2.3 tightened inbox into a triage-first surface with KPI cards, quick filter chips, channel chips, compact AI message hint, localized sender labels and advanced-only CRM linking by raw IDs while preserving conversation assignment, handoff, bot toggle, quick replies, attachments, CRM creation/linking and manual replies. P2.4 kept desktop deals kanban intact while adding production KPI cards, AI risk hint, stage progress bars, mobile stage chips, mobile deal list cards and mobile FAB so deals no longer force a multi-column kanban on phones. P3 changed calendar mobile priority: phones now see horizontal week chips, booking/task counters, AI schedule hint and day agenda before dense calendar chrome, while desktop keeps the richer readiness and day/week/month views. Booking creation, working-hours setup, filters and status actions remain intact.

Frontend production build after production UI/UX P0/P1/P2.1/P2.2/P2.3/P2.4/P3 pass: OK

Owner dashboard production redesign pass: the business-owner dashboard now uses a reference-driven mobile-first production layout instead of the previous oversized multi-section cockpit. The first screen prioritizes revenue, four core KPIs, AI summary, attention actions, lightweight connections, latest leads and today bookings while preserving the existing analytics/onboarding/entity API contracts and manager/operator dashboard flow.
...
Administrator dashboard reference pass: the owner/admin dashboard was reworked toward `references/main_references/administrator_main_desktop+mobile.jpeg`. The first screen now follows the administrator-control composition: greeting and period controls, compact "under control today" metrics, operational KPI row, team/KPI placeholder, attention list, AI summary, latest leads, upcoming bookings and a compact data-connections strip. Mobile uses shorter metric labels and compact cards to match the reference density while preserving existing dashboard data contracts.

Frontend production build after administrator dashboard reference pass: OK
