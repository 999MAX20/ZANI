...
def apply_lead_status(*, lead: Lead, actor, status: str, event_type: str, text: str, request=None, lost_reason=None, clear_lost=False) -> Lead:
    ...
    activity_metadata = {"from": previous_status, "to": status}
    if status == Lead.Statuses.LOST:
        activity_metadata["lost_reason"] = lead.lost_reason
    if clear_lost and previous_lost_reason:
...
def create_appointment_from_lead_contract(*, lead: Lead, actor, service, start_at, resource=None, request=None):
    ...
    previous_status = lead.status
    try:
        appointment = create_appointment_from_lead(lead, service, start_at, resource=resource)
    except ValueError as exc:
        raise ValidationError(str(exc)) from exc
    lead.refresh_from_db()
    if previous_status != lead.status and lead.status not in allowed_lead_status_transitions(previous_status):
        raise ValidationError({"status": f"Cannot move lead from '{previous_status}' to '{lead.status}'."})
    ...
    create_activity_event(
        ...
        metadata={"lead_id": lead.id, "from": previous_status, "to": lead.status},
    )
    if request is not None:
        ...
        write_audit_log(
            ...
            AuditLog.Actions.UPDATE,
            lead,
            metadata={"kind": "lifecycle", "lifecycle_action": "lead_appointment_created", "from": previous_status, "to": lead.status},
        )
