...
class DealStageHistory(TimeStampedModel):
    business = models.ForeignKey(Business, on_delete=models.CASCADE, related_name="deal_stage_history")
    ...
    from_stage = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="deal_stage_history_from",
    )
    to_stage = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="deal_stage_history_to",
    )
    from_status = models.CharField(max_length=32, blank=True)
    to_status = models.CharField(max_length=32, blank=True)
    amount_before = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    amount_after = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    ...
    class Meta:
        ordering = ["-created_at"]
        indexes = [
            models.Index(fields=["business", "deal", "created_at"]),
            models.Index(fields=["business", "from_stage", "to_stage", "created_at"]),
...
class DealValueHistory(TimeStampedModel):
    business = models.ForeignKey(Business, on_delete=models.CASCADE, related_name="deal_value_history")
    deal = models.ForeignKey(Deal, on_delete=models.CASCADE, related_name="value_history")
    amount_before = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    amount_after = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    currency_before = models.CharField(max_length=8, blank=True)
    currency_after = models.CharField(max_length=8, blank=True)
    actor = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="deal_value_history",
    )
    source = models.CharField(max_length=64, blank=True)
    metadata = models.JSONField(default=dict, blank=True)
    ...
    def __str__(self):
        return f"{self.deal_id}: {self.amount_before} {self.currency_before} -> {self.amount_after} {self.currency_after}"
