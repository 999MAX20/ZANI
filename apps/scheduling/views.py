...
class AppointmentViewSet(TenantModelViewSet):
    ...
    def cancel(self, request, pk=None):
        appointment = self.get_object()
        assert_can(request.user, appointment.business, Resources.APPOINTMENTS, Actions.UPDATE, obj=appointment)
        appointment = cancel_appointment(appointment=appointment, actor=request.user, request=request)
        return Response(self.get_serializer(appointment).data)
    ...
    def no_show(self, request, pk=None):
        appointment = self.get_object()
        assert_can(request.user, appointment.business, Resources.APPOINTMENTS, Actions.UPDATE, obj=appointment)
        appointment = mark_appointment_no_show(appointment=appointment, actor=request.user, request=request)
        return Response(self.get_serializer(appointment).data)
