...
class PublicApiAndWebhookTests(TestCase):
    ...
    def test_scoped_token_can_access_public_clients_api(self):
        ...
        self.assertNotEqual(token.token_hash, hashlib.sha256(raw_token.encode("utf-8")).hexdigest())
        self.assertIsNotNone(token.last_used_at)
        self.assertIsNotNone(token.expires_at)
        self.assertLessEqual(token.expires_at, timezone.now() + timezone.timedelta(days=91))

    def test_public_api_accepts_bearer_authorization_header(self):
        self.api.force_authenticate(self.owner)
        create_response = self.api.post(
            "/api/api-tokens/",
            {"business": self.business.id, "name": "Bearer reader", "scopes_json": ["clients:read"]},
            format="json",
        )
        raw_token = create_response.data["token"]
        self.api.force_authenticate(user=None)

        response = self.api.get("/api/public-api/clients/", HTTP_AUTHORIZATION=f"Bearer {raw_token}")

        self.assertEqual(response.status_code, 200)

    def test_public_api_rejects_malformed_authorization_header(self):
    ...
    def test_unknown_token_scope_is_rejected_on_create(self):
        self.api.force_authenticate(self.owner)

        response = self.api.post(
            "/api/api-tokens/",
            {"business": self.business.id, "name": "Unknown scope", "scopes_json": ["clients:read", "admin:all"]},
            format="json",
        )

        self.assertEqual(response.status_code, 400)
        self.assertIn("unsupported", str(response.data).lower())

    def test_api_token_expiration_must_stay_within_max_lifetime(self):
        self.api.force_authenticate(self.owner)
        ...
        response = self.api.post(
            "/api/api-tokens/",
            {
                ...
                "name": "Too long",
                "scopes_json": ["clients:read"],
                "expires_at": (timezone.now() + timezone.timedelta(days=180)).isoformat(),
            },
            format="json",
        )
    ...
    def test_token_rotate_and_revoke(self):
        ...
        self.assertIsNotNone(rotate_response.data["expires_at"])
        self.assertLessEqual(ApiToken.objects.get(id=create_response.data["id"]).expires_at, timezone.now() + timezone.timedelta(days=91))
    ...
    def test_webhook_endpoint_rejects_plain_http_outside_debug(self):
        self.api.force_authenticate(self.owner)

        response = self.api.post(
            "/api/webhook-endpoints/",
            {"business": self.business.id, "name": "Plain HTTP hook", "url": "http://example.com/hook", "events_json": ["system.test"]},
            format="json",
        )

        self.assertEqual(response.status_code, 400)
        self.assertIn("https", str(response.data).lower())
        self.assertEqual(WebhookEndpoint.objects.count(), 0)

    def test_webhook_delivery_rejects_plain_http_without_network_call(self):
        endpoint = WebhookEndpoint.objects.create(
            business=self.business,
            name="Plain HTTP hook",
    ...
    def test_webhook_delivery_sanitizes_error_and_response_body(self):
        endpoint = WebhookEndpoint.objects.create(
            business=self.business,
            name="Unsafe hook",
            url="mock://fail",
        ...
        stored = WebhookDeliveryLog.objects.create(
            business=self.business,
            endpoint=endpoint,
            event_type="system.test",
            idempotency_key="unsafe-response",
            payload_json={},
            response_body='{"access_token":"raw-response-token"}',
            status=WebhookDeliveryLog.Statuses.FAILED,
            error="Failed with api_key=raw-error-key",
        )
        self.api.force_authenticate(self.owner)

        response = self.api.get(f"/api/webhook-deliveries/{stored.id}/")

        self.assertEqual(response.status_code, 200)
        self.assertNotIn("raw-response-token", str(response.data))
        self.assertNotIn("raw-error-key", str(response.data))
