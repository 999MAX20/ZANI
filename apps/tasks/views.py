...
from apps.tasks.selectors import task_workload_payload
...
class TaskViewSet(IdempotentCRMCreateMixin, TenantModelViewSet):
    ...
    def _build_queryset(self, apply_bucket=False):
        ...
        if search:
            queryset = queryset.filter(
                ...
                | Q(conversation__external_user_id__icontains=search)
                | Q(conversation__client__full_name__icontains=search)
        ...
        if relation_filter:
            relation_filters = {
                ...
                "conversation": Q(conversation__isnull=False),
                "none": Q(client__isnull=True, lead__isnull=True, deal__isnull=True, appointment__isnull=True, conversation__isnull=True),
            }
    ...
    def templates(self, request):
        ...
        assert_can(request.user, business, Resources.TASKS, Actions.VIEW)
        return Response(task_templates_payload())
    ...
    def workload(self, request):
        ...
        queryset = scope_queryset(
            Task.objects.filter(business=business).select_related("assignee", "client", "lead", "deal", "appointment", "conversation"),
            request.user,
            business,
            Resources.TASKS,
            Actions.VIEW,
        )
        return Response(task_workload_payload(business=business, queryset=queryset))
    ...
    def _task_details_changes(self, task, validated_data):
        related_fields = {"client", "lead", "deal", "appointment", "conversation", "parent_task", "assignee"}
        changes = {}
