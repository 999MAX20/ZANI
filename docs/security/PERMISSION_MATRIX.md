...
| Specialist date exceptions | `appointments:view`, same Business | `settings:update` for create, update and removal | absence review retains appointment scope and `appointments:update` for manual changes; no automatic cancellation or reassignment | no new client sending; [desired notification flow](../crm/specialist-scheduling.md) |
