...
export function CalendarPage() {
    ...
    return (
      <button
        ...
        className={cn(
          ...
          selectedAppointment?.id === appointment.id && (compact ? "bg-brand-50" : "border-brand-400 bg-brand-50 ring-1 ring-brand-200"),
        )}
  ...
  function renderTaskPreview(task: Task) {
    return (
      <Link
        key={task.id}
      ...
      <section className="space-y-4">
        <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm lg:hidden">
          ...
          <div className="divide-y divide-slate-100">
            {dayAppointments.map((appointment) => renderAppointmentPreview(appointment, true))}
            {dayTasks.map((task) => renderTaskPreview(task))}
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (status === "cancelled" || status === "no_show") {
                            setStatusReasonTarget({ appointment: selectedAppointment, status });
                            setStatusReason("");
                            return;
                          }
