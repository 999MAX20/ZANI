...
class ImportExportTests(TestCase):
    ...
    def test_csv_clients_import_preview_and_confirm(self):
        ...
        self.assertEqual(BusinessEvent.objects.filter(business=self.business, event_type="client.imported").count(), 2)
    ...
    def test_csv_deals_import_creates_open_deals_and_business_events(self):
        upload = SimpleUploadedFile(
            "deals.csv",
            "external_id,title,client_name,phone,email,amount,currency,stage,source,notes\ndeal-1,Imported opportunity,Deal Client,+77010000006,deal@example.com,45000,KZT,New,manual,Needs follow up\n".encode(),
            content_type="text/csv",
        )
        self.api.force_authenticate(self.owner)
        ...
        response = self.api.post(
            "/api/import-jobs/",
            {"business": self.business.id, "entity_type": ImportJob.EntityTypes.DEALS, "source_file": upload},
            format="multipart",
        )
        ...
        self.assertEqual(confirm_response.status_code, 200)
        self.assertEqual(confirm_response.data["imported_count"], 1)
        deal = Deal.objects.get(business=self.business, title="Imported opportunity")
        self.assertEqual(deal.status, Deal.Statuses.OPEN)
        self.assertEqual(deal.amount, 45000)
        self.assertEqual(deal.client.phone, "+77010000006")
