    ...
    <div className="space-y-4">
      ...
      <div className="rounded-card border border-slate-200 bg-white p-4">
        ...
        <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-white">
          ...
          {!previewRows.length ? <p className="px-3 py-4 text-sm text-slate-500">{t("integrations.import.previewAfterCheck")}</p> : null}
        </div>
      </div>
        ...
        <div className="mt-3 space-y-2">
          {jobs.slice(0, 8).map((job) => (
            ...
            >
              ...
              <span className="ml-2 text-slate-500">{importStatusLabel(job.status, t)} · {job.imported_count}/{job.total_rows}</span>
            </button>
          ))}
