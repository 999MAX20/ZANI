...
class Command(BaseCommand):
    help = "Checks local readiness for a real read-only Kaspi orders sync test."

    def add_arguments(self, parser):
        parser.add_argument("--connector-id", type=int, default=0, help="Kaspi BusinessConnector id to validate.")
        parser.add_argument("--format", choices=["text", "json"], default="text")
        parser.add_argument("--fail-on-missing", action="store_true")
        parser.add_argument("--validate", action="store_true", help="Call Kaspi API through the selected connector.")

    def handle(self, *args, **options):
        connector = None
        if options["connector_id"]:
            connector = BusinessConnector.objects.filter(id=options["connector_id"], provider=BusinessConnector.Providers.KASPI).first()
        ...
        if options["fail_on_missing"] and not result["ready_for_local_real_test"]:
            raise CommandError("Kaspi local real-test prerequisites are missing.")

    def _build_checks(self, connector, validate=False):
        checks = [
            {
                "key": "kaspi_enabled",
                "status": "pass" if settings.KASPI_ENABLED else "fail",
                "detail": f"KASPI_ENABLED={settings.KASPI_ENABLED}",
                "action": "Set KASPI_ENABLED=True for real Kaspi API calls.",
            },
            {
            ...
            checks.append(
                {
                    ...
                    "action": "Save Kaspi API token in the connector setup.",
                }
            )
            if validate:
                ...
                checks.append(
                    {
                        "key": "kaspi_api_validation",
                        "status": "pass" if result.get("ok") and not result.get("mock") else "fail",
                        "detail": result.get("reason", "ok") if not result.get("ok") else f"orders_count={result.get('orders_count', 0)}",
                        "action": "Kaspi token must allow read-only orders access.",
                    }
                )
        return checks

    def _write_text(self, result):
        self.stdout.write(f"Kaspi local real-test ready: {result['ready_for_local_real_test']}")
