...
| AUD-027 | ACCESS | CRM cards and pipeline board | Secondary records, client annotations, previews, custom fields and referenced history | VERIFIED_BRANCH (local, uncommitted; not integrated) | [Scoped evidence](../docs/security/aud027-crm-projections.md) |

## Detailed Precedents

### AUD-027 — Parent access disclosed forbidden related CRM data

- Recorded here: 2026-09-16; retain the existing full-product audit ID.
- Type: ACCESS. Status: local verified uncommitted correction; **not CLOSED**.
- Reproduction: operator reads permitted client but receives another owner's
  task and counters; manager's permitted client/shared pipeline exposes an
  out-of-team deal and private field; specialist's permitted appointment card
  includes inaccessible siblings. Direct record lookup correctly denies them.
- Cause: secondary querysets/annotations and card field serialization did not
  use the direct APIs' existing resource/role/sensitive-field restrictions.
- Correction: existing scope/capability checks before arrays, primary fallback,
  next-task preview, counts/limits and recognized entity history; preserve
  permitted data and shared pipeline configuration. No role-system rewrite.
- Evidence: `codex/aud027-access-projections`, base `fc22f727`; initial 8/9
  regression failures and 2/2 list-preview failures, final 189-test scoped PASS,
  17 new regressions, browser role checks and unchanged performance budgets.
- Full source/commands/snapshot/skips/UX observations:
  [AUD-027 record](../docs/security/aud027-crm-projections.md).
- Remaining: accepted integration and its gate; no claim for unrelated generic
  writes, complete UI/UX readiness or production/live integrations.
- Derived rule: a permitted parent must not widen access to its children,
  including indirect disclosure through counts, history, fields and previews.
