...
from apps.automations.engine import process_automation_run, process_due_automation_runs, run_task_overdue_automations
...
@shared_task(bind=True, name="automations.emit_task_overdue_runs", queue="automations")
def emit_task_overdue_runs_task(self, limit=100):
    runs = run_task_overdue_automations(limit=limit)
    return {"emitted": len(runs), "run_ids": [run.id for run in runs if run is not None]}
