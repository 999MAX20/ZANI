...
import { CheckCheck, MessageSquare, PauseCircle, PlayCircle, UserCheck } from "lucide-react";
...
export function ConversationThreadPane({
  ...
  return (
    <WorkQueueDetailPane mobileDetailOpen={mobileThreadOpen} closeLabel={t("common.close")} onMobileClose={onMobileClose}>
      {!selected ? (
        ...
        <>
            ...
            <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Tooltip label={t("conversations.assignTooltip")}>
                  <Button className="h-9 rounded-lg px-3 text-xs" variant="secondary" disabled={!selected} onClick={onAssign} isLoading={assignPending} aria-label={t("conversations.takeDialog")}>
                ...
                {selected.status === "closed" ? (
                  <Tooltip label={t("conversations.reopenTooltip")}>
                    <Button className="h-9 rounded-lg px-3 text-xs" variant="secondary" onClick={onReopenConversation} isLoading={reopenPending} aria-label={t("conversations.openDialog")}>
          ...
          <div ref={messageScrollRef} className="min-h-0 flex-1 space-y-4 overflow-y-auto bg-[linear-gradient(180deg,#fbfcff_0%,#fff7f2_100%)] p-5 pb-28 lg:pb-5">
            {messagesLoading ? <LoadingState label={t("conversations.loadingHistory")} /> : null}
            {canLoadMoreMessages ? (
              <Button
                type="button"
