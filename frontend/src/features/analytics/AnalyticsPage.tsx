  ...
  return (
    <>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        <Stat label="Новые заявки" value={dashboard?.new_leads || 0} hint="требуют обработки" icon={Flame} />
        <Stat label="Сегодня" value={dashboard?.appointments_today || 0} hint="записей на день" icon={CalendarCheck} />
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1fr_1fr]">
        <Card>
          <CardBody>
            ...
            <div className="mt-4 divide-y divide-slate-100">
              {(sourceRows.length ? sourceRows : [{ source: "Нет данных", count: 0 }]).map(({ source, count }) => (
        ...
        <Card>
          <CardBody>
            ...
            <div className="mt-4 divide-y divide-slate-100">
              {[
                ["Новые заявки", dashboard?.new_leads || 0],
                ["Открытые задачи", dashboard?.open_tasks || 0],
                ["Просроченные задачи", dashboard?.overdue_tasks || 0],
                ["No-show", dashboard?.no_show_count || 0],
              ].map(([label, count]) => (
                <div key={label} className="flex items-center justify-between py-3">
                  <div>
                    <p className="font-medium text-slate-700">{label}</p>
                    <p className="text-xs text-slate-400">Откройте соответствующий раздел CRM для обработки.</p>
                  </div>
                  <span className="rounded-full bg-slate-100 px-3 py-1 text-sm font-semibold text-slate-600">{count}</span>
                </div>
              ))}
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <Card>
          <CardBody>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <h2 className="text-lg font-semibold text-midnight">Операционные отчёты</h2>
                <p className="mt-1 text-sm leading-6 text-slate-500">Source ROI, воронка, повторность и команда. Без тяжёлого BI, но с экспортом.</p>
              </div>
              <Button
                variant="secondary"
            ...
            {reportSummary.isLoading ? <div className="mt-4"><LoadingState label="Собираем отчеты..." /></div> : null}
            ...
            {report ? (
              <div className="mt-5 grid gap-4 lg:grid-cols-3">
                <div className="rounded-3xl border border-slate-100 bg-slate-50 p-4">
                  ...
                  <p className="mt-1 text-sm text-slate-500">{report.retention_ltv.repeat_clients} повторных клиентов</p>
                </div>
                <div className="rounded-3xl border border-slate-100 bg-slate-50 p-4">
                ...
                <div className="rounded-3xl border border-slate-100 bg-slate-50 p-4">
                  <p className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">Open deals</p>
        ...
        <Card>
          <CardBody>
            <h2 className="text-lg font-semibold text-midnight">Scheduled reports</h2>
            <p className="mt-1 text-sm leading-6 text-slate-500">Модель расписаний готова. Отправка email будет подключаться через notification/queue слой.</p>
            <div className="mt-4 space-y-3">
              {(scheduledReports.data || []).map((item) => (
                <div key={item.id} className="rounded-2xl border border-slate-100 bg-slate-50 p-3">
                  ...
                  <p className="mt-1 text-xs text-slate-500">{item.recipients_json.join(", ") || "Получатели не заданы"}</p>
                </div>
              ))}
              {!scheduledReports.data?.length ? (
                <div className="rounded-2xl bg-slate-50 p-3 text-sm font-semibold text-slate-500">Расписаний пока нет. Их можно создать через API или будущий UI настроек отчётов.</div>
              ) : null}
            </div>
          </CardBody>
        </Card>
            ...
            <div className="mt-4 divide-y divide-slate-100">
              {(report?.source_roi?.length ? report.source_roi : []).map((row) => (
                <div key={row.source} className="grid gap-3 py-3 md:grid-cols-[1fr_repeat(4,120px)] md:items-center">
                  ...
                  <MiniMetric label="Лиды" value={row.leads} />
                  <MiniMetric label="Записи" value={row.appointments} />
                  <MiniMetric label="Конверсия" value={`${row.conversion_rate}%`} />
                  <MiniMetric label="Выручка" value={row.revenue_estimate} />
                </div>
              ))}
              {!report?.source_roi?.length ? <p className="py-3 text-sm text-slate-500">Источники появятся после первых заявок.</p> : null}
            </div>
          </CardBody>
        </Card>
      </div>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-2">
        <Card>
          <CardBody>
            <h2 className="text-lg font-semibold text-midnight">Воронка по сделкам</h2>
            <div className="mt-4 space-y-3">
              ...
              {!report?.funnel_velocity.deal_stages.length ? <p className="text-sm text-slate-500">Сделки появятся после настройки pipeline.</p> : null}
            </div>
          </CardBody>
        </Card>
      ...
      <div className="mt-6">
        <div className="mb-4 flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-midnight">Team performance</h2>
            <p className="text-sm text-slate-500">Контроль качества команды без тяжелого BI: лиды, handoff, сделки и задачи.</p>
          </div>
        </div>
        {teamPerformance.isLoading ? <LoadingState label="Загружаем показатели команды..." /> : null}
        {teamPerformance.error ? (
          <Card>
            <CardBody className="flex items-start gap-3">
              <ShieldAlert className="mt-1 text-amber-600" size={22} />
              <div>
                <h3 className="font-bold text-midnight">Team analytics скрыта</h3>
                <p className="mt-1 text-sm leading-6 text-slate-500">
                  {getApiErrorMessage(teamPerformance.error) || "Этот раздел доступен владельцу, администратору, руководителю отдела или пользователю с разрешением analytics.view."}
                </p>
              </div>
            </CardBody>
          </Card>
        ...
        {teamPerformance.data ? (
          <div className="space-y-5">
            <div className="grid gap-4 md:grid-cols-3 xl:grid-cols-6">
              <Stat label="Assigned leads" value={teamPerformance.data.totals.assigned_leads} icon={Users} />
              <Stat label="Conversion" value={`${teamPerformance.data.totals.appointment_conversion_rate}%`} icon={TrendingUp} />
              <Stat label="Lost leads" value={teamPerformance.data.totals.lost_leads} icon={TrendingDown} />
              <Stat label="Overdue handoffs" value={teamPerformance.data.totals.overdue_handoffs} icon={AlertTriangle} />
              <Stat label="SLA overdue" value={teamPerformance.data.totals.sla_overdue_deals} icon={Flame} />
              <Stat label="Overdue tasks" value={teamPerformance.data.totals.tasks_overdue} icon={Flame} />
            </div>
            ...
            <div className="grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
              <Card>
                <CardBody>
                  <div className="mb-4 flex items-center justify-between">
                    <h3 className="font-bold text-midnight">Сотрудники</h3>
                    <StatusBadge status={teamPerformance.data.scope} />
                  </div>
                  <div className="space-y-3">
                    {teamPerformance.data.members.map((member) => (
                      <div key={member.user.id} className="rounded-3xl border border-slate-100 bg-slate-50 p-4">
                        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                          ...
                          <div className="grid grid-cols-2 gap-2 text-sm md:grid-cols-4">
                            <MiniMetric label="Лиды" value={member.assigned_leads} />
                            <MiniMetric label="Контакт" value={member.contacted_leads} />
                            <MiniMetric label="Конверсия" value={`${member.appointment_conversion_rate}%`} />
                            <MiniMetric label="Lost" value={member.lost_leads} danger={member.lost_leads > 0} />
                            <MiniMetric label="Задачи" value={member.tasks_overdue} danger={member.tasks_overdue > 0} />
                          </div>
                        </div>
                        <div className="mt-3 grid gap-2 text-xs font-semibold text-slate-500 md:grid-cols-4">
                          ...
                          <span>Closed leads: {member.closed_leads}</span>
                          <span>Appointments: {member.appointments_created}</span>
                        </div>
                      </div>
                    ))}
                    {!teamPerformance.data.members.length ? <p className="text-sm text-slate-500">Нет сотрудников в выбранной области видимости.</p> : null}
                  </div>
                </CardBody>
              </Card>
              ...
              <Card>
                <CardBody>
                  <h3 className="font-bold text-midnight">Action list</h3>
                  <div className="mt-4 space-y-3">
                    {teamPerformance.data.action_items.map((action) => (
                      <Link
                    ...
                    {!teamPerformance.data.action_items.length ? (
                      <div className="rounded-2xl bg-emerald-50 p-3 text-sm font-semibold text-emerald-800">Срочных действий по команде нет.</div>
                    ) : null}
                  </div>

                  <h3 className="mt-6 font-bold text-midnight">Team tab</h3>
                  <div className="mt-4 space-y-3">
                    {teamPerformance.data.teams.map((team) => (
                    ...
                    {!teamPerformance.data.teams.length ? (
                      <div className="rounded-2xl bg-slate-50 p-3 text-sm font-semibold text-slate-500">Команды появятся после настройки отделов.</div>
                    ) : null}
                  </div>
                </CardBody>
              </Card>
