...
Правила выполнения, возобновления и завершения задач определены в
[`AGENTS.md`](../../AGENTS.md). Этот документ владеет инженерными ограничениями;
он не является отдельной очередью задач. Старые локальные пути к исходным
материалам не нужны для выполнения текущей работы.
...
Перед повторной реализацией проверить также существующий task/defect ID,
acceptance criteria, тесты и evidence закрытия. Устаревший план не открывает
закрытую задачу. Новый рефакторинг должен быть необходим для текущего результата
или устранять доказанный риск в изменяемом пути; соседний rewrite не входит в
scope автоматически.
...
Если файл сильно выше лимита, перед добавлением новой логики оценить границы
ответственности. При необходимости для текущей задачи выделить:
...
Правило: если файл превысил лимит и в него добавляется новая логика, сначала
оценить декомпозицию. Это не требование переписывать весь модуль или дробить
работающий код только ради числа строк; независимую очистку согласовать отдельно.
...
Для затронутых областей задача считается завершённой только если:

- использованы существующие слои, а не создан дубль;
- tenant isolation сохранён;
- backend permissions есть на сервере;
- пользовательский flow реально доступен через требуемый UI/API, если задача user-facing;
...
- требуемый для задачи gate из [testing.md](../../docs/testing/testing.md#verification-selection-and-evidence)
  пройден; команды, результаты, snapshot/commit, окружение и пропуски записаны.

Focused checks нужны при итерации, проверки затронутых областей — при закрытии
фазы, полный gate — для интеграции release candidate. Docs-only изменение не
требует install/build или обращения к рабочей базе. Это не освобождает backend
и user-facing изменения от permissions/tenant/regression и flow-проверок.

«Реализовано», «проверено локально», «закоммичено», «интегрировано» и «принято в
целевом окружении» — разные утверждения. Исторический PASS не сертифицирует
последующие изменения. Не снижать acceptance и не скрывать требуемый failed gate.
Применение миграций или seed/reset к существующей рабочей базе не является
обычной тестовой проверкой и требует явного разрешения на конкретную цель.
