...
- Update 2026-08-21: Meta webhook POST authentication is fail closed before payload parsing or tenant/channel resolution. WhatsApp live mode accepts only a valid `X-Hub-Signature-256` HMAC made with a production-strength `WHATSAPP_APP_SECRET`; `X-Zani-WhatsApp-Secret` is restricted to disabled/local mode and must exactly match a configured global or active-channel secret. Instagram always requires a configured `INSTAGRAM_APP_SECRET` or `META_APP_SECRET` and valid HMAC. Missing, weak (in live mode), invalid or unverifiable secrets/signatures must return `403` without persisting messages, events or status changes.
