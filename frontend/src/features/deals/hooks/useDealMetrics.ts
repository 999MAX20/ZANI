...
export function useDealMetrics(
  ...
  const rows = useMemo<DealRow[]>(() => {
    ...
    const ownerMap = new Map(
      data.teamMembers.map((member) => [member.user.id, member]),
    );
    return data.deals
      ...
      .map((deal) => {
        ...
        const risk =
          deal.risk_level && deal.risk_percent !== undefined
            ? { riskLevel: deal.risk_level, riskPercent: deal.risk_percent }
            : dealRisk(deal, tasks);
        const clientEntity =
          data.clientMap.get(deal.client) ||
          (deal.client_name
            ? ({
                id: deal.client,
        ...
        const stageEntity =
          ...
          (deal.stage_name
            ...
            : undefined);
        const ownerEntity = deal.owner
          ? ownerMap.get(deal.owner) ||
        ...
        const nextTask =
          nextOpenTask(tasks) ||
          (deal.next_task_id && deal.next_task_title
            ? ({
      ...
      .filter((deal) => {
        const searchable = [
          deal.title,
          deal.source,
          deal.clientEntity?.full_name,
          deal.clientEntity?.phone,
          deal.clientEntity?.email,
        ]
          .join(" ")
          .toLowerCase();
        ...
        return (
          (filters.statusFilter === "all" ||
            deal.status === filters.statusFilter) &&
          (!filters.ownerFilter ||
            String(deal.owner || "") === filters.ownerFilter) &&
          (filters.stageFilter === "all" ||
            String(deal.stage) === filters.stageFilter) &&
          ...
          (!filters.dateFrom ||
            created >= new Date(filters.dateFrom).getTime()) &&
          (!filters.dateTo ||
            created <= new Date(filters.dateTo).getTime() + 86_399_999) &&
          (!searchValue || searchable.includes(searchValue)) &&
          (filters.quickFilter === "all" ||
            (filters.quickFilter === "mine" &&
              Boolean(user?.id && deal.owner === user.id)) ||
            (filters.quickFilter === "hot" && deal.riskPercent >= 60) ||
            (filters.quickFilter === "overdue" && Boolean(deal.sla_overdue)) ||
            (filters.quickFilter === "no_tasks" &&
              deal.status === "open" &&
              !deal.nextTask &&
              !deal.next_action_at))
        );
  ...
  const model = useMemo<DealMetricsModel>(() => {
    ...
    const noTaskDeals = openDeals.filter(
      (deal) => !deal.nextTask && !deal.next_action_at,
    );
    ...
    return {
      ...
      quickFilters: [
        ...
        {
          value: "mine",
          label: t("deals.filterMine"),
          count: pipelineRows.filter(
            (deal) => user?.id && deal.owner === user.id,
          ).length,
        },
        { value: "hot", label: t("deals.hot"), count: staleDeals.length },
        {
          value: "overdue",
          label: t("deals.overdue"),
          count: overdueDeals.length,
        },
        {
          value: "no_tasks",
          label: t("deals.noTasksFilter"),
          count: noTaskDeals.length,
        },
      ],
