...
качество ответов, billing и рассылка не входят. Полный локальный gate и UI PASS;
это не закрытие всего AI-блока. Публикация/actual CI, контракт и проверки —
в [primary checkpoint](../testing/task-state/PRIMARY-SESSION.md).

Last updated: 2026-09-22 (V1-A03–A09 staff confirmation; historical closures preserved).
...
исторические границы этапа. Финансовый контракт/UI прошли полный gate на
`cc7050e`; точный публикационный SHA, readback и CI — в квитанции checkpoint.
Это не подключение реальной учётной системы и не общий допуск клиники.

## Локальная CRM и допуск клиники — 2026-09-21

Текущий ограниченный пакет V1-F06 / V1-W02: специалист без обязательного логина,
собственный недельный график, исключения по датам и ручной разбор записей при
...
Update 2026-07-24: B-201 aligned pilot-critical mutation side effects across direct API, Inbox, automation and approved AI entry points. Inbox/automation/AI task creation now uses the shared task service with tenant-validated relations, taxonomy activity, sanitized entity audit and routed notification output. Requestless trusted domain actors can write the same sanitized assignment/create audits as HTTP actors. Conversation assignment, handoff, unread, priority, close and reopen actions use taxonomy constants; replaying an unchanged action produces no new activity, audit, notification or system-message noise. Automation retries and wait/resume transitions update one `automation_run` activity row instead of appending one row per attempt.
