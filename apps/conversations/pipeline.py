...
def _ensure_client(*, conversation: BotConversation, created: dict[str, bool], actor, source: str, qualification: ConversationQualification | None = None) -> Client:
    ...
    create_activity_event(
        ...
        metadata={"conversation_id": conversation.id, "channel": conversation.channel},
    )
    return client
...
def _ensure_deal(
    *,
    ...
    create_activity_event(
        ...
        metadata={"conversation_id": conversation.id, "lead_id": lead.id if lead else None, "channel": conversation.channel},
    )
    return deal
