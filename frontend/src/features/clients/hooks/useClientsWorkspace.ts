...
import { segmentsApi, taggedObjectsApi, tagsApi } from "../../../api/activities";
import type { CrmCardTab, CrmDrawerEntity } from "../../../components/crm/CrmEntityDrawer";
import type { Client, Id, Segment, Tag, TaggedObject } from "../../../types";
import { useClientRows } from "./useClientRows";
import { useDebouncedValue } from "./useDebouncedValue";
import type { ClientQuickFilter, ClientTableColumn, ClientTag, SegmentDraft, Translate } from "../types";
...
export function useClientsWorkspace({
  ...
  const filteredClients = useQuery({
    queryKey: ["clients", "filtered", businessId, debouncedSearch, source, selectedTag, selectedSegment, quickFilter, page, pageSize],
    queryFn: () =>
  ...
  const taggedObjects = useQuery<TaggedObject[]>({
    queryKey: ["tagged-objects", "clients", businessId, clientIdList, RELATED_PAGE_SIZE],
  ...
  const selectClient = useCallback((clientId: number) => openClientCard(clientId), [openClientCard]);
