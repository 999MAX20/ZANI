...
def archive_instance(request, instance, reason=""):
    ...
    write_audit_log(request, AuditLog.Actions.UPDATE, instance, metadata={"archive": True, "reason": reason or ""})
    write_activity_event(request, f"{instance.__class__.__name__.lower()}.archived", instance, text=f"Archived: {reason or 'no reason'}")
    return instance
...
def restore_instance(request, instance):
    ...
    instance.save(update_fields=["is_archived", "archived_at", "archived_by", "archive_reason", "updated_at"])
    write_audit_log(request, AuditLog.Actions.UPDATE, instance, metadata={"restore": True})
    write_activity_event(request, f"{instance.__class__.__name__.lower()}.restored", instance, text="Restored from archive")
    return instance
