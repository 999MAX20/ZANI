...
export function DealsPage() {
  ...
  const quickCounts = useMemo(
    () => ({
      ...
      hot: summary.data?.hot ?? rows.filter((deal) => deal.riskPercent >= 60).length,
      overdue: summary.data?.overdue ?? rows.filter((deal) => deal.sla_overdue).length,
      no_tasks: summary.data?.no_tasks ?? rows.filter((deal) => deal.status === "open" && !deal.nextTask && !deal.next_action_at).length,
    }),
    [data.deals.length, rows, summary.data?.hot, summary.data?.mine, summary.data?.no_tasks, summary.data?.overdue],
  );
  ...
  const archiveMutation = useMutation({
    ...
    onSuccess: async (_, variables) => {
      ...
      showNotification({ message: t("deals.noticeArchived", { count: variables.ids.length }), tone: "success" });
      showUndoToast({
        message: t("deals.noticeArchived", { count: variables.ids.length }),
        onUndo: async () => {
          await Promise.all(variables.ids.map((id) => dealsApi.restore(id)));
          await queryClient.invalidateQueries({ queryKey: ["deals"] });
          showNotification({ message: t("deals.noticeRestored", { count: variables.ids.length }), tone: "success" });
        },
      });
    },
  });
  ...
  async function requestArchiveSelectedDeals() {
    ...
    const result = await confirmAction({
      ...
      description: t("deals.archiveSelectedText", { count: selection.selectedIds.length }),
    ...
    archiveMutation.mutate({ ids: selection.selectedIds, reason: result.reason });
  }
  ...
  useEffect(() => {
    function onKeyDown(event: KeyboardEvent) {
      ...
      if (event.key === "Delete" && selection.selectedIds.length) void requestArchiveSelectedDeals();
    }
